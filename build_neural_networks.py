...
class BuildActorNetwork:
    def __init__(self, state, scope):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            self.actions_out_unscaled = tf.compat.v1.layers.dense(
                inputs=self.layer,
            ...
            self.action_scaled = tf.multiply(
                0.5,
                tf.multiply(self.actions_out_unscaled, Settings.ACTION_RANGE)
        ...
        with tf.compat.v1.variable_scope(self.scope):
            with tf.compat.v1.variable_scope("Training"):
                ...
                self.actor_gradients = tf.gradients(
                    ys=self.action_scaled, xs=self.parameters, grad_ys=-dQ_dAction
                )

                # The actor gradients are summed over the batch, so we must divide by
                # the batch size to get the mean gradients.
                self.actor_gradients_scaled = list(
                    map(
                        lambda x: tf.divide(x, Settings.MINI_BATCH_SIZE),
                ...
                actor_training_function = self.optimizer.apply_gradients(
                    zip(self.actor_gradients_scaled, self.parameters)
                )
    ...
    def __init__(self, state, action, scope):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            if Settings.LEARN_FROM_PIXELS:
                # Build convolutional layers
                for i, conv_layer_settings in enumerate(Settings.CONVOLUTIONAL_LAYERS):
                    self.state_side = tf.compat.v1.layers.conv2d(
                        inputs=self.state_side,
                        activation=tf.nn.relu,
                        name="state_conv_layer" + str(i),
                        **conv_layer_settings
                    )  # the "**" allows the passing of keyworded arguments
            ...
            for i, number_of_neurons in enumerate(Settings.CRITIC_HIDDEN_LAYERS):
                self.state_side = tf.compat.v1.layers.dense(
                    inputs=self.state_side,
                    units=number_of_neurons,
                    activation=None,
            ...
            for i, number_of_neurons in enumerate(Settings.CRITIC_HIDDEN_LAYERS[1:]):
                self.action_side = tf.compat.v1.layers.dense(
                    inputs=self.action_side,
                    units=number_of_neurons,
                    activation=None,
            ...
            self.q_distribution_logits = tf.compat.v1.layers.dense(
                inputs=self.layer,
            ...
            self.q_distribution = tf.nn.softmax(
                self.q_distribution_logits, name="output_probabilities"
            )

            # The value bins that each probability corresponds to.
            self.bins = tf.linspace(
            ...
            self.dQ_dAction = tf.gradients(
                ys=self.q_distribution, xs=self.action, grad_ys=self.bins
            )  # also known as action gradients

    def generate_training_function(
        self, target_q_distribution, target_bins, importance_sampling_weights
    ):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            with tf.compat.v1.variable_scope("Training"):
                ...
                self.optimizer = tf.compat.v1.train.AdamOptimizer(
                    Settings.CRITIC_LEARNING_RATE
                )

                # Project the target distribution onto the bounds of the original network
                projected_target_distribution = l2_project(
                    target_bins, target_q_distribution, self.bins
                )

                # Calculate the cross entropy loss between the projected distribution and the main q_network!
                self.loss = tf.nn.softmax_cross_entropy_with_logits(
                    logits=self.q_distribution_logits,
                    labels=tf.stop_gradient(projected_target_distribution),
                )
                ...
                critic_training_function = self.optimizer.minimize(
                    self.total_loss, var_list=self.parameters
                )
...
def l2_project(z_p, p, z_q):
    ...
    d_neg = tf.compat.v1.where(
        d_neg > 0, 1.0 / d_neg, tf.zeros_like(d_neg)
    )  # 1 x Kq x 1
    d_pos = tf.compat.v1.where(
        d_pos > 0, 1.0 / d_pos, tf.zeros_like(d_pos)
    )  # 1 x Kq x 1
    ...
    return tf.reduce_sum(
        input_tensor=tf.clip_by_value(1.0 - delta_hat, 0.0, 1.0) * p, axis=2
    )
