...
class Settings:
    ...
    VIDEO_RECORD_FREQUENCY = (
        20  # Multiples of "CHECK_GREEDY_PERFORMANCE_EVERY_NUM_EPISODES"
    )
    ...
    RESUME_TRAINING = (
        False  # If True, be sure to set "RUN_NAME" to the previous run's filename
    )
    ...
    REPLAY_BUFFER_START_TRAINING_FULLNESS = (
        0  # how full the buffer should be before training begins
    )
    MINI_BATCH_SIZE = 256
    ...
    if UNIFORM_OR_GAUSSIAN_NOISE:
        ...
        NOISE_SCALE = (
            1 / 3
        )  # standard deviation = 1/3 the action range. Therefore a 3-sigma action will cause full exploration in the worst case scenario
    ...
    SAVE_CHECKPOINT_EVERY_NUM_ITERATIONS = (
        100000  # how often to save the neural network parameters
    )
    ...
    if ENVIRONMENT == "gym":
        env = environment_file.Environment(
            ...
            0,
            CHECK_GREEDY_PERFORMANCE_EVERY_NUM_EPISODES,
            VIDEO_RECORD_FREQUENCY,
    ...
    NORMALIZE_STATE = (
        env.NORMALIZE_STATE
    )  # Normalize state on each timestep to avoid vanishing gradients
    MIN_V = env.MIN_V
    MAX_V = env.MAX_V
    ...
    STATE_MEAN = (LOWER_STATE_BOUND + UPPER_STATE_BOUND) / 2.0
    STATE_HALF_RANGE = (UPPER_STATE_BOUND - LOWER_STATE_BOUND) / 2.0
