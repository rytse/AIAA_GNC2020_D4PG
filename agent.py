...
environment_file = __import__(
    "environment_" + Settings.ENVIRONMENT
...
class Agent:
    ...
    def create_summary_functions(self):
        ...
        timestep_number_summary = tf.compat.v1.summary.scalar(
            "Agent_" + str(self.n_agent) + "/Number_of_timesteps",
            self.timestep_number_placeholder,
        )
        episode_reward_summary = tf.compat.v1.summary.scalar(
            "Agent_" + str(self.n_agent) + "/Episode_reward",
            self.episode_reward_placeholder,
        )
        self.regular_episode_summary = tf.compat.v1.summary.merge(
            [timestep_number_summary, episode_reward_summary]
        )

        # If this is agent 1, the agent who will also test performance, additionally log the reward
        if self.n_agent == 1:
            test_time_episode_reward_summary = tf.compat.v1.summary.scalar(
                "Test_agent/Episode_reward", self.episode_reward_placeholder
            )
            test_time_timestep_number_summary = tf.compat.v1.summary.scalar(
                "Test_agent/Number_of_timesteps", self.timestep_number_placeholder
            )
            self.test_time_episode_summary = tf.compat.v1.summary.merge(
                [test_time_episode_reward_summary, test_time_timestep_number_summary]
            )
    ...
    def build_actor(self):
        ...
        )  # the * lets Settings.STATE_SIZE be not restricted to only a scalar
    ...
    def build_actor_update_operation(self):
        ...
        for source_variable, destination_variable in zip(
            source_variables, destination_variables
        ):
    ...
    def run(self, stop_run_flag, replay_buffer_dump_flag, starting_episode_number):
        ...
        print(
            "Starting to run agent %i at episode %i."
            % (self.n_agent, starting_episode_number[self.n_agent - 1])
        )
        ...
        while (
            episode_number <= Settings.NUMBER_OF_EPISODES and not stop_run_flag.is_set()
        ):
            ####################################
            ...
            test_time = (self.n_agent == 1) and (
                episode_number % Settings.CHECK_GREEDY_PERFORMANCE_EVERY_NUM_EPISODES
                == 0
                or episode_number == 1
            )
            ...
            if test_time and Settings.TEST_ON_DYNAMICS:
                self.agent_to_env.put(
                    (True, test_time)
                )  # Reset into a dynamics environment only if it's test time and desired
            else:
                self.agent_to_env.put(
                    (False, test_time)
                )  # Reset into a kinematics environment
            ...
            if test_time:
                ...
                noise_scale = (
                    Settings.NOISE_SCALE * Settings.NOISE_SCALE_DECAY**episode_number
                )
            ...
            while not done:
                ...
                action = self.sess.run(
                    self.policy.action_scaled,
                    feed_dict={self.state_placeholder: np.expand_dims(state, 0)},
                )[
                    0
                ]  # Expanding the state to be a 1x3 instead of a 3
                ...
                if Settings.UNIFORM_OR_GAUSSIAN_NOISE:
                    ...
                    exploration_noise = (
                        np.random.uniform(
                            low=-Settings.ACTION_RANGE,
                            high=Settings.ACTION_RANGE,
                            size=Settings.ACTION_SIZE,
                        )
                        * noise_scale
                    )
                else:
                    ...
                    exploration_noise = (
                        np.random.normal(size=Settings.ACTION_SIZE)
                        * Settings.ACTION_RANGE
                        * noise_scale
                    )  # random number multiplied by the action range

                # Add exploration noise to original action, and clip it incase we've exceeded the action bounds
                action = np.clip(
                    action + exploration_noise,
                    Settings.LOWER_ACTION_BOUND,
                ...
                (
                    ...
                    *guidance_position,
                    ...
                    self.env_to_agent.get()
                )  # The * means the variable will be unpacked only if it exists
                ...
                if (
                    ...
                    and not Settings.ENVIRONMENT == "gym"
                ):
                    if not done:
                ...
                if Settings.NORMALIZE_STATE:
                    ...
                    next_state = (
                        next_state - Settings.STATE_MEAN
            ...
            if (
                ...
                print(
                    "Rendering Actor %i at episode %i" % (self.n_agent, episode_number)
                )

                os.makedirs(
                    os.path.dirname(
                        Settings.MODEL_SAVE_DIRECTORY + self.filename + "/trajectories/"
                    ),
                    exist_ok=True,
                )
                np.savetxt(
                    ...
                    + str(episode_number)
                    + ".txt",
                    np.asarray(raw_state_log),
                )

                # Ask the learner to tell us the value distributions of the state-action pairs encountered in this episode
                self.agent_to_learner.put(
                    (
                        np.asarray(state_log),
                        np.asarray(action_log),
                        np.asarray(next_state_log),
                        np.asarray(instantaneous_reward_log),
                        np.asarray(done_log),
                        np.asarray(discount_factor_log),
                    )
                )
                ...
                try:
                    (
                        ...
                        loss_log,
                    ) = self.learner_to_agent.get(timeout=3)

                    bins = np.linspace(
                        Settings.MIN_V, Settings.MAX_V, Settings.NUMBER_OF_BINS
                    )

                    # Render the episode
                    environment_file.render(
                        np.asarray(raw_state_log),
                        np.asarray(action_log),
                        desired_pose,
                        np.asarray(instantaneous_reward_log),
                        np.asarray(cumulative_reward_log),
                        critic_distributions,
                        target_critic_distributions,
                        projected_target_distribution,
                        bins,
                        np.asarray(loss_log),
                        np.squeeze(np.asarray(guidance_position_log)),
                        episode_number,
                        self.filename,
                        Settings.MODEL_SAVE_DIRECTORY,
                    )
            ...
            if (
                episode_number % Settings.DISPLAY_ACTOR_PERFORMANCE_EVERY_NUM_EPISODES
                == 0
            ):
                print(
                    "Actor "
                    + str(self.n_agent)
                    + " ran "
                    + str(Settings.DISPLAY_ACTOR_PERFORMANCE_EVERY_NUM_EPISODES)
                    + " episodes in %.1f minutes, and is now at episode %i"
                    % ((time.time() - start_time) / 60, episode_number)
                )
                start_time = time.time()
            ...
            feed_dict = {
                self.episode_reward_placeholder: episode_reward,
                self.timestep_number_placeholder: timestep_number,
            }
            if test_time:
                summary = self.sess.run(
                    self.test_time_episode_summary, feed_dict=feed_dict
                )
            else:
                summary = self.sess.run(
                    self.regular_episode_summary, feed_dict=feed_dict
                )
        ...
        print(
            "Actor %i finished after running %i episodes!"
            % (self.n_agent, episode_number - 1)
        )
